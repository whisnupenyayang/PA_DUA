...
import 'dart:convert';
import 'detail_pengelolaan.dart';
import 'package:belajar_flutter/connection.dart'; // Import file connection.dart

class PengelolaanPage extends StatefulWidget {
  @override
  _PengelolaanPageState createState() => _PengelolaanPageState();
}

class _PengelolaanPageState extends State<PengelolaanPage> {
  ...
  Future<List<dynamic>> _fetchDataUsers() async {
    var result = await http.get(Uri.parse(Connection.buildUrl(
        '/budidaya/pengelolaan_penaung'))); // Menggunakan Connection.buildUrl()
                            ...
                            ListTile(
                              onTap: () {
                                Navigator.push(
                                  ...
                                  MaterialPageRoute(
                                    builder: (context) =>
                                        DetailPengelolaanPage(data: data),
                                  ),
                                );
                              },
                              leading: ClipRRect(
